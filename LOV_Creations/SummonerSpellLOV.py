def summoners_lov(connection, lol_watcher):
    cur = connection.cursor()
    DDRegion = lol_watcher.data_dragon.versions_for_region('na1')['n']
    DDsummoner = DDRegion['summoner']
    summoners_response = lol_watcher.data_dragon.summoner_spells(DDsummoner)
    patch_version = summoners_response['version']
    summoners = summoners_response['data']
    summoner_dict = {}
    cur.execute(''' TRUNCATE TABLE "lollov".summoner_spells ''')
    for n in summoners:
        icon_url = f"https://ddragon.leagueoflegends.com/cdn/{patch_version}/img/spell/{summoners[n]['image']['full']}"
        summoner_dict.update({'spell_name' : summoners[n]['name'],
                                'spell_id' : summoners[n]['key'],
                                'modes' : summoners[n]['modes'],
                                'icon_url' : icon_url})
        cur.execute('''INSERT INTO "lollov".summoner_spells
        (spell_name, spell_id, modes, icon_url)
        VALUES
        (%(spell_name)s, %(spell_id)s, %(modes)s, %(icon_url)s);''', summoner_dict)
    connection.commit()
    print(f"Summoner Spells processed.")
